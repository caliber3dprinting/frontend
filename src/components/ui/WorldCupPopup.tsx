    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="relative w-full max-w-lg bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 shadow-2xl">
              ...
              {/* Header */}
              <div className="bg-zinc-950 px-5 pt-5 pb-4 text-center border-b border-zinc-800">
                <p className="text-xs font-bold tracking-[0.2em] text-orange-500 uppercase mb-1">
                  ⚽ Edición Especial Mundial 2026 🏆
                </p>
                <h2 className="text-2xl font-black uppercase tracking-wide text-white leading-tight">
                  Copa del Mundo
                </h2>
                <p className="text-zinc-500 text-xs mt-1">Impresión 3D · Alta calidad · Envío disponible</p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 p-3">
                {PRODUCTS.map((p) => (
                  <div
                    key={p.id}
                    className="group relative bg-zinc-800 rounded-xl overflow-hidden border border-zinc-700 hover:border-orange-500 transition-all duration-200"
                  >
                    <div className="relative aspect-[3/4] w-full">
                      ...
                      />
                      <span className="absolute top-1.5 left-1.5 text-[10px] font-bold bg-orange-500 text-white px-1.5 py-0.5 rounded-full uppercase tracking-wide leading-none">
                        {p.tag}
                      </span>
                    </div>
                    <div className="p-2 text-center">
                      <p className="text-zinc-300 text-[11px] leading-snug font-medium">{p.name}</p>
                      {p.size && <p className="text-zinc-500 text-[10px]">{p.size}</p>}
                      <p className="text-orange-500 font-black text-sm mt-0.5">{p.price}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="px-4 pb-4 pt-1 text-center">
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#1db954] text-white font-bold px-5 py-3 rounded-xl transition-colors text-sm"
                >
